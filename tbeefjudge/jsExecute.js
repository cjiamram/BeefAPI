...
function validInput(){
		var flag=true;
		flag=regDec.test($("#obj_beeGrade").val());
		if (flag==false){
			$("#obj_beeGrade").focus();
			return flag;
}
...
function displayData(){
		var url="tbeefjudge/displayData.php?tableName=t_beefjudge&dbName=dbbeefjudge&keyWord="+$("#txtSearch").val();
		$("#tblDisplay").load(url);
}
function createData(){
		var url='tbeefjudge/create.php';
		jsonObj={
			judgeCode:$("#obj_judgeCode").val(),
			beefCode:$("#obj_beefCode").val(),
			beeGrade:$("#obj_beeGrade").val(),
...
function readOne(id){
		var url='tbeefjudge/readOne.php?id='+id;
		data=queryData(url);
		if(data!=""){
			$("#obj_judgeCode").val(data.judgeCode);
			$("#obj_beefCode").val(data.beefCode);
			$("#obj_beeGrade").val(data.beeGrade);
...
function confirmDelete(id){
		...
		if (willDelete.value) {
			url="tbeefjudge/delete.php?id="+id;
